...
void PushResult(int i, int j, int** A, int* stones, vector<int>& res)
{
    if (A[i][j] == 0)
        return;
    else if (A[i - 1][j] == A[i][j])
        PushResult(i - 1, j, A, stones, res);
    else
    {
        PushResult(i - 1, j - stones[i - 1], A, stones, res);
...
void solve(int N, int W, int* stones, vector<int>& res)
{
    ...
    for(int i = 1;i <= N; i++)
    {
        ...
        {
            A[i][j] = A[i - 1][j];
            if (j >= stones[i - 1] && ( A[i - 1][j - stones[i - 1]] + stones[i - 1] > A[i][j]))
                A[i][j] = A[i - 1][j - stones[i - 1]] + stones[i - 1];
        }
    }

    PushResult(N, W, A, stones, res);
